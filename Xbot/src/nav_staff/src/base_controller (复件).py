...
class BaseController:
 def __init__(self):
  ...
  rospy.Subscriber('/clicked_point', PointStamped, self.GoalCB, queue_size=1)
 ...
 def define(self):
  ...
  #self.pathsteps = Queue.Queue(maxsize = 1)
  self.pathsteps = list()
 ...
 def OdomCB(self, odom):
  #self.locker.acquire()
  #print 'odomcb: len path ', self.pathsteps.qsize()
  #if self.pathsteps.qsize():
   #path = self.pathsteps.get()
  if len(self.pathsteps):
   path = copy.deepcopy(self.pathsteps)
   self.MoveBase(odom, path)
  #self.locker.release()
 ...
 def PlanCB(self, PlanPath):
  #self.locker.acquire()
  #print 'plancb', len(PlanPath.poses)
  if len(PlanPath.poses) > 0:
   if not len(self.GoalDic): #如果有path，没有goal，初始化goal
    ...
    OriGoal.header.stamp = rospy.Time.now()
    self.update(OriGoal)
   path = list()
   path = self.Pathhandle(PlanPath)
   self.pathsteps = copy.deepcopy(PlanPath.poses)
   #self.pathsteps.put(path.poses)
  #print 'PlanCB self.pathsteps', self.pathsteps.qsize()
  #self.locker.release()


 def MoveBase(self, odom, path):
  cmd = Twist()
  #print 'odom: len path ', len(path)
  if len(path):
   if len(path) > self.num :
    cmd = self.DiffControl(odom, path[self.num].pose, path, cmd)
    #print 'get cmd'
   else: # situation of len(path) < self.num
    if self.ArrivedPosition(odom, path):
 ...
 def update(self, goal):
   self.seq += 1
   CurrentGoal = PointStamped()
   CurrentGoal = goal
   self.GoalDic[self.seq] = CurrentGoal


 def Pathhandle(self, PlanPath):
  Pathdb = list()
  # check if plan start from current position if yes copy path, otherwise set new goal
  CurrentPosition = rospy.wait_for_message('%s' % self.OdomTopic, Pose)
  PositionCheck_result = self.PositionCheck(PlanPath.poses[0], CurrentPosition)
  #print 'Pathhandle position check result:', PositionCheck_result
  # 在路径规划范围之内
  if PositionCheck_result:
   Pathdb = PlanPath.poses
  # 在路径规划范围之外
  else:
   NewGoal = PointStamped()
   NewGoal.point = PlanPath.poses[0].pose.position
   NewGoal.header.seq = PlanPath.header.seq
 ...
 def GetCommand(self, GoalAngle, OdomAngle):
  ...
  #如果不同边（存在符号更变）：只需要以最快速度过界，从而达到同边即可
   ...
   if OCriticalLine == GCriticalLine:
    #rospy.loginfo('GetCommand: reg as 不同边,同临界线')
    # 不同边, pi 临界线 临界角
    if OCriticalLine == numpy.pi:
     rospy.loginfo('GetCommand: 不同边, pi 临界线 临界角')
     if GoalAngle >= 0:
      rospy.loginfo('GetCommand: 不同边, pi 临界线 临界角 GoalAngle >= 0')
    ...
    # 不同边,0 临界线 临界角
     ...
     if GoalAngle >= 0:
      rospy.loginfo('GetCommand: 不同边,0 临界线 临界角 GoalAngle >= 0')
      OdomToC = (abs(GoalAngle) + abs(OdomAngle))
     else:
      rospy.loginfo('GetCommand: 不同边,0 临界线 临界角 GoalAngle < 0')
      OdomToC = -(abs(GoalAngle) + abs(OdomAngle))
   ...
   # 不同边, 不同临界线
    ...
    if OdomToCPP + GoalToCPP < OdomToCPO + GoalToCPO:
     #rospy.loginfo('GetCommand: 不同边, 不同临界线')
     if OdomAngle >= 0:
      rospy.loginfo('GetCommand: 不同边, 不同临界线 OdomAngle >= 0')
    ...
    # 不同边, 0 临界线
     ...
     rospy.loginfo('GetCommand: 不同边, 0 临界线 ')
     OdomToC = (0 - OdomAngle)
 ...
 def DiffControl(self, odom, goal, path, cmd):
  ...
  # 是否当前线位移运动量在豁免范围之外，是则进行线运动
  if linear > self.PathBias:
   #是否当前角位移运动量在豁免范围之外，是则进行角运动
   if abs(cmdtwist) >= self.AngularBias:
    ...
    #只有当差角小于AngularBias时候才移动
    #rospy.loginfo('DiffControl: AngularBias: small circle')
    cmd.angular.z = cmdtwist
    cmd = self.linear_cmd_gen(linear, cmd)
 ...
 def linear_cmd_gen(self, linear, cmd):
  # 进行线运动设置
